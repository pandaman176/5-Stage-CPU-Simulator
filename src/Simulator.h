...
inline bool hasRs1(Inst inst) {
  if (inst != LUI && inst != AUIPC && inst != JAL) {
    return true;
  }
  return false;
}

inline bool hasRs3(Inst inst) {
  if (inst == FMADDI || inst == FMADDU || inst == FMSUBI || inst == FMSUBU || 
      inst == FNMSUBI || inst == FMNADDI) {
    return true;
  }
  return false;
}

inline bool hasRs2(Inst inst) {
    if (inst == JAL || inst == AUIPC || inst == LUI || 
        inst == JALR || 
        inst == LB || inst == LH || inst == LW || inst == LD || 
        inst == LBU || inst == LHU || inst == LWU ||
        inst == ADDI || inst == SLTI || inst == SLTIU || 
        inst == XORI || inst == ORI || inst == ANDI || 
        inst == SLLI || inst == SRLI || inst == SRAI ||
        inst == ADDIW || inst == SLLIW || inst == SRLIW || inst == SRAIW) {
        return false;
    }
    return true;
}
